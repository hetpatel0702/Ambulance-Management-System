...
google.maps.event.addDomListener(window, 'load', initialize);
    ...
    function setaddress_db(add){
        var add_db=add;
        document.cookie = "address="+add_db;
    }
</script>
...
<main class="login-body">
    <form class="form-default" action="details_php.php" method="POST">
        ...
        <div class="login-form" style="padding-top:50px;">
            ...
            <?php
                $sql1 = "SELECT * FROM route WHERE ptID=".$_SESSION['userID'];
                $result1 = mysqli_query($conn,$sql);
                if(mysqli_num_rows($result1) > 0){
                    echo '<script >setaddress_db('.$rows['Address'].');</script>';
                    echo'<div class="form-input" style="">
                            <input type="submit" name="submit" value="Open Route">
                        </div>';
                }else{
